...
export const metadata: Metadata = {
  ...
  openGraph: { title: 'Public Holidays NZ 2026 & 2027 | Dates & Observed Days', description: 'All New Zealand public holiday dates for 2026 and 2027, plus observed dates and regional anniversary days.', url: '/public-holidays', type: 'website' },
};
...
export default async function PublicHolidaysPage() {
  ...
  const faq = [
    { q: 'What are the next public holidays in New Zealand?', a: 'The table on this page marks every 2026 and 2027 national holiday as Past, Today or Upcoming using New Zealand time, so you can quickly see which public holidays are still ahead.' },
    { q: 'How many public holidays are there in New Zealand?', a: 'There are 12 public holidays each year when the relevant provincial anniversary day is included.' },
  ...
  const jsonLd = { '@context':'https://schema.org', '@graph':[ { '@type':'WebPage', name:'Public Holidays NZ 2026 & 2027', url:'https://webfitnews.com/public-holidays', description:metadata.description, dateModified:snapshot.checkedAt, isPartOf:{'@type':'WebSite',name:'Webfit News',url:'https://webfitnews.com'} }, { '@type':'FAQPage', mainEntity:faq.map(item=>({'@type':'Question',name:item.q,acceptedAnswer:{'@type':'Answer',text:item.a}})) } ] };

  return <>
    <SiteHeader/>
    <main className={`shell ${styles.page}`}>
      <section className={styles.hero}>
        <div><span className={styles.eyebrow}>New Zealand Public Holiday Calendar</span><h1>Public Holidays NZ 2026 & 2027</h1><p className={styles.lead}>All New Zealand national public holiday dates, observed days and regional anniversary holidays for 2026 and 2027, with live Past, Today and Upcoming status.</p><div className={styles.freshness}><span className={snapshot.sourceStatus==='live'?styles.liveDot:styles.fallbackDot}/><strong>Official sources checked:</strong> {formatNZDate(snapshot.checkedAt)}</div></div>
        <div className={styles.heroCard}><span>Live NZ calendar</span><strong>Past · Today · Upcoming</strong><small>Status updates automatically using Pacific/Auckland time.</small></div>
      </section>

      <section className={styles.section}><div className={styles.sectionHeading}><span className={styles.kicker}>Quick answer</span><h2>NZ public holidays at a glance</h2><p>This page lists every national public holiday for 2026 and 2027, including Mondayised or Tuesdayised observed dates where they apply. It also includes Auckland, Wellington, Canterbury and other regional anniversary holidays.</p></div></section>
      ...
      {[2026,2027].map(year=><section key={year} id={String(year)} className={styles.section}>
        <div className={styles.sectionHeading}><span className={styles.kicker}>National dates</span><h2>NZ public holidays {year}</h2><p>Past holidays are de-emphasised, today is highlighted, and future dates are marked upcoming. Observed dates are used for status where Mondayisation or Tuesdayisation applies.</p></div>
        <NationalHolidayTable year={year} holidays={snapshot.years[year]}/>
      </section>)}

      <section id="anniversary" className={styles.section}>
        <div className={styles.sectionHeading}><span className={styles.kicker}>Regional holidays</span><h2>NZ regional anniversary days 2026 & 2027</h2><p>Anniversary days vary by region. Use the tables below for Auckland, Wellington, Canterbury, Otago, Southland and the other regional observed dates.</p></div>
      ...
      <section id="sources" className={styles.section}><div className={styles.sectionHeading}><span className={styles.kicker}>Official sources</span><h2>Official NZ public holiday sources</h2><p>Webfit News checks official New Zealand government sources and presents the dates in a simpler calendar format. Government pages remain the authoritative source.</p></div><div className={styles.sourceList}>{publicHolidaySources.map(source=><a key={source.url} href={source.url} target="_blank" rel="noopener noreferrer"><div><strong>{source.name}</strong><small>{source.primary?'Primary calendar source':'Official cross-check'}</small></div><span>Open official page ↗</span></a>)}</div><div className={styles.metaRow}><span>Last checked: {formatNZDate(snapshot.checkedAt)}</span><span>{snapshot.sourcesChecked} of {publicHolidaySources.length} official pages responded</span></div></section>
