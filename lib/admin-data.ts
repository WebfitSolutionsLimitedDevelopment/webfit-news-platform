...
export async function getAdsAdmin(){const supabase=await createClient();const [{data:slots},{data:campaigns}]=await Promise.all([supabase.from('ad_slots').select('*').order('label'),supabase.from('ad_campaigns').select('*').order('created_at',{ascending:false})]);return {slots:slots||[],campaigns:campaigns||[]};}
