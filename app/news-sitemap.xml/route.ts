...
export async function GET(){
  ...
  const urls=recent.map(s=>`<url><loc>${escapeXml(`${SITE_URL}/${s.slug.trim()}/`)}</loc><news:news><news:publication><news:name>Webfit News</news:name><news:language>en</news:language></news:publication><news:publication_date>${new Date(s.published_at!).toISOString()}</news:publication_date><news:title>${escapeXml(s.title)}</news:title></news:news></url>`).join('');
