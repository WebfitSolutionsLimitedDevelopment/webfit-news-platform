...
export async function PublicFooter(){
  ...
  return <footer className="site-footer-premium"><div className="shell">
    <div className="footer-top"><div className="footer-brand"><img src="/webfit-news-logo.png" alt={siteName}/><p>{tagline}</p></div><div className="footer-news"><strong>News</strong><Link href="/category/new-zealand">New Zealand</Link><Link href="/category/auckland">Auckland</Link><Link href="/category/politics">Politics</Link><Link href="/category/business">Business</Link><Link href="/category/immigration">Immigration</Link></div><div className="footer-about"><strong>Webfit News</strong><Link href="/about">About</Link><Link href="/editorial-policy">Editorial Policy</Link><Link href="/corrections">Corrections</Link><Link href="/privacy-policy">Privacy Policy</Link><Link href="/terms">Terms</Link><Link href="/contact">Contact</Link><Link href="/advertise-media-kit">Advertise</Link><Link href="/support-us">Support Webfit News</Link><Link href="/login">Reader Sign In</Link><Link href="/rss.xml">RSS</Link></div><div className="footer-social"><strong>Follow</strong>{social.length?social.map(([name,url])=><a key={name} href={url} target="_blank" rel="noopener noreferrer">{socialLabels[name]||name.replaceAll('_',' ')}</a>):<span>Social links can be added in Newsroom Settings.</span>}</div></div>
